...
para modelizar la relación de la edad y el sexo con el número de días por semana que una persona lee un periódico. Ajustamos el modelo 
...
Como puedes ver, esto implica que procedemos en dos pasos: primero, utilizamos algunos datos para entrenar a nuestro modelo. A 
...
En la siguiente sección, describiremos los términos y conceptos clave del aprendizaje automático. A continuación, hablaremos de modelos 
...
En resumen, en cualquier escenario en el que se cumplan estas dos condiciones previas: en primer lugar, tenemos un gran conjunto de 
...
lugar, para un subconjunto aleatorio de los datos (digamos $2.000$ de los titulares), ya conocemos la clase. Por ejemplo, porque los 
...
de que disponga (por ejemplo, si se trata de un problema de clasificación binario en lugar de multiclase), y de lo distribuidos que 
...
Una vez que hayamos comprobado que disponemos de este conjunto de datos etiquetados y nos hayamos asegurado de que es de buena 
...
lo son realmente, entonces la precisión $\frac{150}{150+50} = \frac{150}{200} = 0.75$.
...
A modo de breve repaso: $P(A|B)$ puede leerse como: la probabilidad de A, dado B. O: la probabilidad de A si B es el 
...
característica $x$. Como puedes imaginar, hacer esto no lleva mucho tiempo, que es lo que hace que el clasificador Naive Bayes sea una 
...
si tenemos un conjunto de mensajes de las redes sociales y queremos modelar el medio del que probablemente proceden, entonces su longitud 
...
Ante el reto de predecir si un mensaje de las redes sociales es un tuit o un post de Facebook, podríamos predecir "post de Facebook" si 
...
Los árboles de decisiones tienen dos buenas propiedades. En primer lugar, son muy fáciles de explicar. De hecho, una figura como la de 
...
Sin embargo, esto tiene un precio. Formular un modelo como una serie de preguntas sí/no, como puede imaginarse, pierde intrínsecamente 
...
decisión equivocada al principio del árbol (es decir, cerca de su nodo raíz), no se puede corregir después. Esta rigidez hace que los 
árboles de decisión también sean propensos al sobreajuste: pueden ajustarse muy bien a los datos de entrenamiento, pero no generalizar 
lo suficiente a datos (de prueba) ligeramente diferentes.
...
"voten" sobre la etiqueta que debemos predecir. Este procedimiento se denomina "votación por mayoría", pero también existen otros 
...
que se activan si la entrada total supera un determinado umbral.
...
propagación hacia delante, lo que significa que cada neurona de una capa está conectada a cada neurona de la capa siguiente. En el 
...
y otros modelos en los recursos especializados citados anteriormente.
...
Las redes neuronales convolucionales son una de las formas en que el aprendizaje profundo puede superar esta limitación. Esencialmente, 
...
001 para "*recommend*"). En la segunda columna, una ventana de desplazamiento concatena estos valores en pares de palabras (así 010001 
...
*spam*. Esto nos daría un filtro de *spam* más conservador, con probablemente una mayor precisión a costa de un menor exhaustividad.
...
Por desgracia, si calculamos la precisión y la exhaustividad (o cualquier otra métrica) de varios modelos en el mismo conjunto de datos 
...
Si nuestro clasificador generaliza bien, esperaríamos que nuestra métrica de interés (por ejemplo, la exactitud, o el valor $F_1$) sea 
...
Ten en cuenta que, en R, no todos los parámetros son "sintonizables" utilizando 'caret' estándar. Por lo tanto, una réplica exacta de 
las búsquedas de cuadrícula en el [@exm-gridsearch] y [@exm-gridsearch2] requeriría comparaciones manuales o escribir una extensión de 
'caret'.
...
tiene que serlo, necesariamente. En R, por el contrario, no puedes (al menos, no fácilmente) poner cualquier parámetro del modelo en la 
cuadrícula. En su lugar, tienes buscar los "parámetros sintonizables" que deben estar presentes como parte de la rejilla en la 
