...
'networkx'). Se pueden añadir y eliminar nodos y aristas a o desde este grafo inicial, o incluso modificar los nombres de los vértices. Una 
...
con Twitter? ¿Puedo representar las relaciones entre usuarios de la misma forma que en Facebook? Pues bien, cuando se modelan grafos es 
sumamente importante tener una definición clara de lo que se quiere decir con los nodos y las aristas, para mantener una interpretación 
...
Ejemplo [-@exm-directed], las direcciones se representan con el orden del par de nodos: la primera posición es para el "desde" y la segunda 
...
unidireccionales. Puedes visualizar este grafo dirigido en el Ejemplo [-@exm-visdirected] y ver cómo las aristas contienen ahora flechas 
útiles.
...
los valores por 0.001 para hacerlo realista en la pantalla, pero también podrías normalizar estos valores si fuera necesario). 
También incluimos la variable 'party' (partido), que posteriormente es recodificada como 'color' para representar a los republicanos con el 
...
denominado *clique* (clan o camarilla), que es un subconjunto independiente o completo de un grafo no dirigido (cada nodo del clique 
...
Donald Trump y el más largo incluye los nodos Sanders, Clinton y Trump.
...
#| results: hide
# Elimina la amistad entre Clinton y Trump
g6 = g1.copy()
...
también podemos identificar sus nodos adyacentes, o lo que es lo mismo, sus vecinos. En el mismo ejemplo, utilizamos 'neighbours' (la misma 
...
# Densidad en la red de Facebook
nx.density(g1)
```
```{python density-python2}
# Densidad en la red de Twitter
nx.density(g2)
```
```{python density-python3}
# Transitividad en la red de Facebook
nx.transitivity(g1)
```
```{python density-python4}
# Diámetro en la red de Facebook
nx.diameter(g1, e=None, usebounds=False)
```
```{python density-python5}
# Reciprocidad en la red de Twitter
nx.reciprocity(g2)
...
dirigidos, tienes tres opciones para medir el grado. En primer lugar, según el número de aristas que apuntan hacia un nodo, lo que 
...
# Grado de centralidad de la red de Facebook (no dirigido):
print(nx.degree_centrality(g1))
```
```{python centrality1-python2}
# Grado de centralidad de la red de Twitter (dirigido):
print(nx.degree_centrality(g2))
```
```{python centrality1-python3}
# Grado de centralidad de las entradas de la red de Twitter (dirigido):
print(nx.in_degree_centrality(g2))
```
```{python centrality1-python4}
# Grado centralidad de las salidas de la red de Twitter (dirigido):
...
# Centralidad de cercanía de la red de Facebook (no dirigida)
print(nx.closeness_centrality(g1))
```
```{python centrality2-python2}
# Centralidad de autovector de la red de Facebook (no dirigida)
print(nx.eigenvector_centrality(g1))
```
```{python centrality2-python3}
# Centralidad de intermediación de la red de Facebook (no dirigida)
...
# Nota: ve al resultado de R para ver el gráfico
```
## Código R
```{r girvan-r}
...
# Louvain:
# (Fíjate en que el resultado de Louvain es un diccionario de 
# {member: cluster} en vez de una lista de clústeres, por lo que 
...
# Greedy optimization:
# (Ten en cuenta que el resultado nxcom es un generador, 
# ordenarlo lo convierte en una lista)
...
#| results: hide
# ¿De qué cluster nace cada nodo?
# (Ten en cuenta que para los clústeres de Louvain, se invierte el preprocesamiento 
# que hicimos antes, por lo que podríamos haber utilizado el resultado 
# community_louvain(.)  original. Para las operaciones de la red comunitaria sí 
...
fijamos en las centralidades grado, intermediación y proximidad podemos obtener fácilmente algunos de los otros nodos relevantes de la red 
