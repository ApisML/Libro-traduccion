...
codificar sea manifiesta y concreta en lugar de latente y abstracta: nombres de personas, objetos físicos específicos, frases concretas, 
...
ello y es bastante improbable que los discursos sobre otros temas contengan una expresión como "control de armas" que confunda al proceso. 
...
con otras palabras como "bolsa", "mercado", "empresa"... Por desgracia, nos encontraremos rápidamente con un problema al que ya nos 
...
económico, la hace el ordenador en cuestión de segundos. El aprendizaje automático supervisado, por tanto, ha sustituido a los enfoques 
...
qué categorías existen (positivas frente a negativas, deportes frente a economía o política &hellip;). La gran ventaja de los 
...
no saber que el tema "economía" existe. Además, en la práctica, la asignación  a priori de los temas resultantes del modelo temático a las 
categorías existentespuede suponer un reto.
...
[@exm-reviewdata], que también sirve para mostrar cómo descargar y procesar archivos. Tanto R como Python siguen el mismo 
...
'reviewdata.rds'. En Python, podemos conseguir los archivos del material descargado directamente, por lo que no necesitamos extraerlo. 
...
tener en cuenta la negación: "no es bueno" se contará como positivo. Tampoco podemos tener en cuenta modificadores como "muy bueno". Además, todas 
...
un buen argumento para utilizarlos cuando es importante que el método no sea difícil de entender, sino totalmente transparente incluso sin 
...
Para determinar el número de documentos que se necesitan para calcular el acuerdo entre un humano y una máquina, se pueden seguir las 
...
Recapitulemos los elementos básicos que necesitamos. En el capítulo [-@sec-chap-introsml], aprendimos a utilizar distintos clasificadores, 
a evaluarlos y a elegir la mejor configuración. Sin embargo, en estos ejemplos, utilizamos datos numéricos como características, no texto. 
Por suerte, en el [@sec-chap-dtm], aprendiste a convertir texto en características numéricas: vamos a empezar con eso.
...
todo, los "sentimientos" no son exactamente conceptos claros y manifiestos para los que podamos enumerar una lista de palabras. Se ha 
...
conjunto de datos: entrenar un clasificador en, por ejemplo, críticas de películas y luego usarlo para predecir los sentimientos en textos 
...
pero también podemos añadir restricciones adicionales, como excluir palabras muy raras o muy comunes), así como podríamos asignar un 
identificador de uso interno (el nombre de la variable) a cada palabra. Si ajustamos de nuevo el clasificador, estas ya no serían 
...
siempre). En concreto, en nuestro caso, la regresión logística mejora la excesiva clasificación errónea de las reseñas positivas como 
...
Una *pipeline* de este tipo se presta muy bien a realizar una gridsearch. El [@exm-gridsearchlogreg] lo demuestra. Con 
...
palabras extremadamente infrecuentes y las extremadamente frecuentes, e incluimos tanto unigramas como bigramas (lo que, podemos especular, 
nos ayuda a tener en cuenta el problema de "nada bueno" frente a "nada", "bueno"), y cambiamos la penalización por defecto de $C=1$ a 
$C=100$.
...
de 0 es un problema notorio en textos muy cortos), la precisión y la exhaustividad son claramente peores incluso que las del modelo base 
simple con el que empezamos, y mucho peores que las del modelo final con el que terminamos. De hecho, pasamos por alto ¡la mitad! de las 
...
*Latent Dirichlet Allocation*) [LDA, @blei03]. Aunque el objetivo de la LDA es el mismo que el de las técnicas de clústeres, parte del otro 
extremo, por lo que se denomina un modelo generativo. Un modelo generativo es un modelo formal (simplificado) de cómo se supone que se han 
...
El modelo generativo en el que se basa la LDA funciona de la siguiente manera. Supongamos que eres periodista y escribes una noticia de 500 
...
embargo, este modelo generativo ayuda a comprender la interpretación sustantiva de los temas. Además, la LDA es un modelo mixto, es decir, 
...
situación es la contraria: conocemos los documentos y queremos conocer los temas. Por lo tanto, la tarea de la LDA es encontrar los 
...
Un método de cálculo que se utiliza a menudo para la LDA es el muestreo de Gibbs. En pocas palabras, comienza con una asignación aleatoria 
...
muchos temas diferentes, optamos por dividirlo primero por párrafos, ya que estos serán más coherentes semánticamente (al menos en el 
...
sentido normal de ese término, pero a todas ellas hay que asignarles un tema en la LDA. Por lo tanto, a menudo se ven temas "de 
...
El ejemplo [-@exm-ldaresults] muestra cómo combinar los resultados de la LDA (temas por documento) con los metadatos del documento 
...
### Más allá de la LDA {#sec-beyondlda}
...
gran cantidad de variaciones y extensiones de la LDA. Entre ellas se incluyen, por ejemplo, los modelos temáticos dinámicos (que incorporan 
...
[^1]: La perplejidad es una medida para comparar y evaluar modelos temáticos utilizando la log-verosimilitud con el fin de calcular lo bien que un modelo predice una muestra. Consulta la nota "¿Cuántos temas?" a continuación para ver un código de ejemplo sobre cómo calcular la perplejidad.
...
Existen varias métricas de este tipo propuestas en la literatura académica, de las que presentaremos dos. La “perplejidad” (*perplexity*) es una 
