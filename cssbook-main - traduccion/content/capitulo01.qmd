...
de las ciencias sociales. Se explica por qué le uso de Python y 
...
al otro y sin que ninguno reclame superioridad [@margolin19]. Así, la caja de 
...
pregunta teóricamente interesante cuando analizamos el proceso en su conjunto, 
...
Lo que realmente esperamos es que los contenidos de este libro proporcionen 
...
Una técnica que algunos lectores pueden echar en falta es el modelado basado en agentes 
(ABM, por sus siglas en inglés). Por una parte, estas técnicas de simulación son 
importantes en las ciencias sociales computacionales en general [@cioffi-revilla2014], 
y recientemente se han aplicado también al análisis de la comunicación 
[@Waldherr2014;@Wettstein2020]. Sin embargo, al revisar los planes de estudio de 
los cursos actuales que enseñan el análisis computacional de la comunicación, 
...
[p. ej., @Rieder2017], o el movimiento del texto como dato [@Grimmer2013], vemos un 
conjunto de técnicas que se utilizan una y otra vez, y que por lo tanto hemos incluido 
en nuestro libro. En concreto, además de las técnicas generales de análisis y 
visualización de datos, se trata de técnicas de recopilación de datos como el *web 
...
importante en qué idioma se hace.
...
esperamos que esté familiarizado con los capítulos anteriores que 
...
científicos sociales que deseen refrescar sus conocimientos deberían poder 
...
y jugueteando. Por este motivo, los ejemplos de código son, probablemente, 
...
código Python. Todas estas herramientas están disponibles y bien documentadas para 
Windows, MacOS y Linux. Una vez explicado cómo instalar R y Python, hay una sección 
muy importante sobre la instalación de paquetes. Si planeas usar solo R o Python 
(por ahora), siéntete libre de saltarte la parte sobre el otro lenguaje.

Si quieres escribir programas Python más largos (a diferencia de, por ejemplo, 
...
para realizar ciencia de datos. Anaconda es gratuita, código abierto y está concebida 
...
que necesita para seguir los ejemplos de este libro e incluye entornos de desarrollo 
...
se indica en este capítulo? Depende. A su favor, al descargar Anaconda se tiene todo 
...
paquetes pueden necesitarlo). En sus contras, es enorme y también instala muchas cosas 
...
Haz clic en el enlace para instalar R para tu sistema operativo, e instala la última versión. 
...
en el que esté trabajando), y siempre cambiar a un proyecto cuando abras RStudio
...
guardar en un fichero, o pasar directamente a consola o a un documento R.
...
estado haciendo. Sin embargo, si solo tienes una instantánea del espacio de trabajo, 
...
Algunos paquetes, como `tidyverse` (R) y `pandas` (Python), dedicados el tratamiento de datos, se utilizan en 
casi todos los capítulos. Muchos capítulos también introducen paquetes específicos, como `igraph`/`networkx` 
para el análisis de redes en el @sec-chap-network. Para facilitar llevar la cuenta de los paquetes necesarios 
...
Las notas también incluyen un bloque de código para importar todos los paquetes usados en el capítulo, 
