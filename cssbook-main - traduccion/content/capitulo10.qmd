...
::: {.callout-warning}
# Actualización pendiente: R Tidytext

En el momento de escribir este capítulo, utilizamos `quanteda` para realizar el análisis de texto en R.
A día de hoy, considetamos que el paquete `tidytext` es más sencillo de aprender para estudiantes que 
ya lo han usado para manipulación de datos.
Por lo tanto, vamos a reescribir este capítulo utilizando ese paquete.
Puedes consultar [relevant github issue](https://github.com/vanatteveldt/cssbook/issues/5) para tener más información.
:::
...
Como puedes ver, el Ejemplo [-@exm-dtm] muestra una DTM elaborada a partir de dos líneas del famoso poema de Maya Angelou. La matriz 
...
primera línea de entrada, este tokeniza por defecto "*haven't*" a 'haven', que tiene un significado radicalmente diferente (*´t* indica negación, 
...
También puedes escribir tu propio tokenizador si lo prefieres. Por ejemplo, el 'TreebankTokenizer' asume que el texto ya ha sido 
...
para procesar las frases, manteniendo solo los tokens que incluyen al menos un carácter de letra. Aunque un poco más complicado, este 
...
'CountVectorizer' del mismo modo que anteriormente usamos el 'TreebankWordTokenizer' para el inglés. Del mismo modo, en el caso de 
...
El ejemplo [-@exm-sotu] muestra un ejemplo más realista. Descarga todos los discursos del "Estado de la Unión" de Estados Unidos. y crea una 
...
En el Ejemplo [-@exm-freq], mostramos cómo se puede ver el contenido de la DTM. En primer lugar, calculamos las frecuencias globales de 
término y de documento de cada palabra, donde visualizamos palabras con diferentes frecuencias. Como era de esperar, la palabra “*the*” 
...
Sin embargo, las 1.000 palabras con la máxima frecuencia siguen utilizándose en menos de la mitad de los documentos. Dado que en el corpus 
...
eficiente (véase, sin embargo, la Sección [-@sec-workflow] para problemas relacionados con el aprendizaje automático en matrices dispersas 
en R).
...
facilidad de uso. Por esa razón, la DTM en Python no "recuerda" las palabras reales, sino que utiliza el índice de cada palabra, lo que 
...
En Python, no existe una función que transforme directamente el texto en una DTM. En su lugar, se crea un transformador llamado 
...
utilizarse para transformar otros datos en una DTM con exactamente las mismas columnas, lo que a menudo es necesario para los algoritmos. 
...
tareas: la aparición frecuente de palabras como "empleo", "estupendo" o "yo" pueden ser buenos indicadores de que un texto trata sobre 
...
los análisis automáticos, es importante entender qué información está buscando el ordenador, ya que este no puede encontrar patrones en 
...
palabras pueden ser aleatorios, variar en función de la frecuencia de las palabras o tener una forma decorativa.
...
esto se hace utilizando la función 'textplot_wordcloud' de 'quanteda'. En Python, es un poco más complejo, ya que solo se tienen los 
...
palabras como "*a*", "*and*" o "*the*". Hay que fijarse bien para encontrar palabras como "*federal*" o "*security*", que dan pistas 
...
palabras no son informativas para responder nuestras preguntas. Esto es evidente si se observa una nube de palabras (que no es más que 
...
frecuencia de una palabra específica en ese documento. El término “vector” también aparece en el nombre del tokenizador de 'scikit-learn': 
un vectorizador o función para convertir textos en vectores.

En este caso, "vector" puede interpretarse como una palabra elegante para un grupo de números. Como ya sabes, el término se utiliza a menudo 
...
dimensionalidad del vector. Esta es también una interpretación muy útil de los vectores en el análisis de textos: la dimensionalidad del 
...
ruido: eliminación de palabras vacías; limpieza de puntuación y otros artefactos, y recorte y ponderación. Para los ejemplos de esta sección, 
...
tuits en un marco de datos que contiene el ID y el texto de los tuits. Como puedes ver, este conjunto de datos contiene muchos elementos
no textuales, como hipervínculos y hashtags, así como signos de puntuación y palabras vacías. Antes de empezar a analizar 
estos datos, tendremos que decidir y realizar varios pasos de limpieza.

::: {.callout-note appearance="simple" icon=false}

::: {#exm-trumptweets}
Principales palabras utilizadas en los tuits de Trump
...
muestra el resultado de eliminar las palabras vacías inglesas en ambos paquetes. ¡Atrévete a probarlo con otros idiomas, 
...
Es más, para algunas preguntas de investigación, determinadas palabras vacías pueden ser muy relevantes. Si lo que te interesa son 
...
Por esta razón, siempre es una buena idea entender y revisar qué lista de palabras vacías estás utilizando, y utilizar una diferente o 
...
de las preguntas de investigación. Por ejemplo, puede contener emoticonos o *emojis*, *hashtags* de Twitter o menciones, así como etiquetas HTML 
u otras anotaciones.
...
En general, si solo deseas conservar o eliminar determinadas palabras, suele ser más fácil hacerlo después de la tokenización utilizando 
una expresión regular para seleccionar las palabras que quieres conservar o eliminar. Si deseas eliminar partes de palabras (por ejemplo, 
...
antes de la tokenización. Del mismo modo, si quieres eliminar un término que sería dividido por la tokenización (como los hipervínculos), 
puede ser mejor hacerlo antes de que se produzca la tokenización.
...
Las expresiones regulares se explican detalladamente en la Sección [-@sec-regular], por lo que, aquí, seremos breves: la barra '|' divide el 
...
::: {#exm-tagcloud}
Limpieza del corpus y creación de una nube de etiquetas
...
crear el corpus, tokenizar, mantener solo los *hashtags*, y crear una DFM. Para darle un poco más de gracia al resultado, utilizamos el 
...
el entorno de trabajo con funciones que de todos modos no se pretende reutilizar. La desventaja es que lo que está pasando resulta menos 
claro, al menos para las personas no familiarizadas con las funciones lambda.
...
todos modos, no tienen apenas relevancia. Sin embargo, el recorte de una DTM a, por ejemplo, al menos un 1\% de frecuencia de documento 
...
documentos”, en castellano. Pondera cada aparición de una palabra por su frecuencia bruta (frecuencia de términos) corregida por la 
frecuencia con la que aparece en todos los documentos (frecuencia inversa de documentos). En una fórmula, la aplicación más habitual 
...
ponderados resultantes con los resultados presentados allí. Como puedes ver, la ponderación tf$\cdot$idf en ambos lenguajes tiene más o 
...
En todos los ejemplos anteriores, se han creado matrices documento-término en las que cada columna representaba una palabra. Sin embargo, en 
...
exclusivamente las palabras de forma individual, sino combinaciones de palabras o grupos de palabras relacionadas. Esto suele denominarse 
ingeniería de características (*feature engineering*), ya que estamos utilizando nuestra experiencia en el campo para encontrar las 
características adecuadas (columnas, variables independientes) y así captar el significado relevante para nuestra pregunta de investigación. 
Si utilizamos otras columnas además de palabras, también es más correcto, técnicamente, utilizar el nombre “matriz documento-característica”, 
...
característica es un par de palabras adyacentes. En el ejemplo anterior, “*the movie was not bad*”, se producen los siguientes bigramas: 
the-movie, movie-was, was-not y not-bad. Cada uno de esos bigramas se trata como un rasgo, es decir, una DTM contendría una columna por 
...
posturas políticas, pero, por ejemplo, un bigrama como "*not good*" también sería útil para el análisis de sentimientos (pero asegúrate de 
...
palabras que aparecen con más frecuencia de la esperada en función de su aparición subyacente. Por ejemplo, la expresión "*crystal clear*" 
...
obstante, ambos métodos arrojan resultados muy similares, con Saddam Hussein, el Telón de Acero, Al Qaida y “*red tape*” encabezando la 
...
obtener las palabras más similares a la palabra “*fraud*” (fraude), que aparentemente está relacionada con términos similares como 
...
a un sustantivo. Además, conocer la etiqueta POS de una palabra puede ayudar a hacerla menos ambigua. Por ejemplo, “*like*” como verbo, 
...
es *(to) read* y el lema del sustantivo *books* (libros) es *book*. En castellano, reduciríamos los verbos a su infinitivo (de salto, 
...
relativamente sencillas y limitadas, por lo que el *stemming* puede producir resultados adecuados. Sin embargo, en el caso de lenguas más 
...
Las relaciones sintácticas pueden ser útiles si se necesita diferenciar entre quién hace algo y a quién se lo hacen. Por ejemplo, uno 
...
atacar y ser atacado era importante ([@clause]). Aunque lo más probable es que, en la mayoría de los casos, no necesites esta información 
...
ejemplo, profundizando en los patrones de uso por persona o a lo largo del tiempo, o utilizando un modelado de temas no supervisado para 
...
Como alternativa a 'UDPipe', puedes utilizar 'Spacy', que es otro conjunto de herramientas para procesar el lenguaje natural gratuito y bastante 
...
mano. Nosotros recomendamos especialmente 'Stanza' (@stanza), ya que es una herramienta muy útil y flexible, con soporte para múltiples 
...
frecuencia en el documento y/o ponderación 'tf.idf'. Recuerda que cuando utilices una lista de palabras vacías, siempre debes 
