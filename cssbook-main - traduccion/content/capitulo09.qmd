...
Este capítulo utiliza paquetes para el tratamiento de datos textuales. Para R, se trata principalmente del paquete 'stringr' 
...
codificación/descodificación de/a Unicode). En esta sección, cubrimos los pasos típicos de la limpieza de texto, como el uso de minúsculas y la 
...
evaluar cuáles son las mejores opciones. Por ejemplo, en algunos caso,s mantener las mayúsculas en una conversación de chat o en un 
...
texto en inglés, es muy importante que te asegures de que entiendes Unicode y sus codificaciones, y que compruebes que los textos con los que 
trabajas están decodificados correctamente. Consulta la sección [-@sec-encodings] para obtener más información sobre cómo funciona todo esto.
...
# Nota: usa collapse para pasar a valor único
...
necesitas aplicar una operación, tienes que usar uno de los métodos de pandas que se muestran a continuación o utilizar un bucle *for* o una 
...
ceñirse a 'stringr'. En particular, stringr es muy similar a 'stringi' (y de hecho está parcialmente basada en ese paquete). Así, por poner un 
...
como un objeto de caracter o cadena (también como listas o diccionarios, pero igualmente tendrán cadenas dentro de ellos), lo que significa 
...
gran cantidad de ruido. La Tabla [-@tbl-stringoperations] resume algunas operaciones útiles sobre cadenas en R y Python que te ayudarán 
en esta etapa.
...
relativamente sencilla (aunque no del todo correcta) para localizar una dirección de correo electrónico es '[\w\.-]+@[\w\.-]+\.\w\w+', 
...
En esencia, las expresiones regulares son patrones que identifican secuencias de caracteres. En el caso más sencillo, una “letra regular” 
...
También puedes especificar la frecuencia con la que debe aparecer cada carácter o grupo de caracteres. Por ejemplo, 'a+' significa 
...
un punto. La solución es anular la función de estos símbolos especiales anteponiéndoles una barra invertida ('\'): 'a\+' coincide con el 
...
mes de un solo dígito. Ten en cuenta que estos patrones no comprueban la validez de las fechas, pero un añadido sencillo sería 
...
correcta que coincida con direcciones de correo electrónico puede tener más de 400 caracteres! Peor aún, probablemente ni siquiera sea posible 
...
text = """   <b>Communication</b>    
    (del Latin, communicare, que significa “compartir”) """
# elimina las etiquetas:
cleaned = re.sub("<[^>]+>", "", text)
# normaliza los espacios en blanco
...
cosas: se puede dividir una cadena en varias subcadenas, se puede extraer varias coincidencias de una cadena y se pueden unir varias 
coincidencias.
...
# Tres maneras de conseguir lo mismo:
items = text.split(", ")
