...
La primera limpieza suele ser eliminar las columnas innecesarias y renombrar las columnas con nombres poco claros o demasiado 
largos. En particular, suele ser conveniente renombrar las columnas que contienen espacios o caracteres no estándar, para que 
sea más fácil referirse a ellas más adelante.
...
'select: select(d, column1=col1, column2)'. Con ello se renombra 'col' a 'column1', mantiene esa columna y 'column2', al tiempo que 
...
ya que no ofrece tantas ventajas sobre la asignación directa.
...
escuela; o, a partir de un archivo de datos que contenga información por artículos, podríamos calcular el número medio de menciones 
a un político por día (¡Cada fecha podría tener múltiples artículos y cada artículo múltiples menciones a políticos!).
...
::: {#exm-aggregate}
Agregación. Observa que en el ejemplo de Python, podemos especificar funciones frecuentes tales como "'mean'" en una cadena de caracteres, pero también podríamos pasarlas como funciones numéricas directamente, como 'np.mean' de 'numpy'
...
generalmente una función o un método (véase la sección [-@sec-functions]). En R, cada línea solía tener la forma: 
...
::: {#exm-aggregate2}
Combinar varias funciones o métodos. El resultado es idéntico al Ejemplo [-@exm-aggregate].
...
regulares, tal y como se muestra en el ejemplo: primero se calcula la media por grupo, seguida de la desviación de esta media.
...
En muchos casos, éste es el comportamiento deseado, ya que ambos marcos de datos pueden contener, por ejemplo, una columna “Year” o 
...
recibió cada candidato a las primarias presidenciales de 2016 en cada condado: Bernie Sanders obtuvo 544 votos en el condado de 
Autauga, en el estado estadounidense de Alabama, lo que supuso el 18,2\% de todos los votos emitidos en las primarias demócratas. 
...
::: {.callout-note appearance="simple" icon=false}
::: {#exm-primary}
 Resultados de las primarias de 2016 y metadatos a nivel de condado. Ten en cuenta que para evitar la duplicados, mostramos los datos de los condados en el ejemplo de Python y los datos de los resultados en el ejemplo de R.
...
de los datos correspondientes a los condados. Te proponemos un pequeño puzzle: ¿Crees que puedes utilizar los comandos de filtrado de 
...
causal directa, ya que puede haber factores subyacentes, como la fecha de las elecciones (que es muy importante en las primarias). Desde 
el punto de vista estadístico, dado que las observaciones dentro de un estado no son independientes, lo que deberíamos controlar es el 
voto a nivel a nivel de cada estado. Por ejemplo, podríamos utilizar una correlación parcial, pero seguiríamos violando el supuesto de 
independencia de los errores, por lo que sería mejor adoptar un enfoque de modelización más sofisticado (por ejemplo, multinivel). Sin 
embargo, esto queda fuera del alcance de este capítulo.
...
Generalmente, los datos con múltiples observaciones de la misma unidad se denominan datos anchos (ya que hay muchas columnas), mientras 
que un conjunto de datos con una fila por cada observación se denomina datos largos (ya que hay muchas filas). En la mayoría de los 
casos, es más fácil trabajar con datos largos y, de hecho, en la jerga de 'tidyverse', estos datos se denominan “*tidy data*” 
(datos ordenados).
...
estas variables. De hecho, cuando anteriormente fusionamos estos conjuntos de datos en el Ejemplo [-@exm-merge2], seleccionamos solo 
...
con el archivo de Excel, que incluye filas de encabezado adicionales y nombres de columnas destinados al consumo humano en lugar de 
a la computación fácil.
...
El paso que falta, dividir una cabecera en dos columnas, se realiza utilizando 'separate' (R) o 'split' (Python).
