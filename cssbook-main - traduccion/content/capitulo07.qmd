...
El análisis exploratorio de datos (EDA, por sus siglas en inglés), tal y como fue concebido originalmente por @tukey1977exploratory, 
puede ser una manera excelente de preparar y evaluar datos, así como para comprender sus propiedades y generar ideas en cualquier fase 
de la investigación. Es prácticamente obligatorio realizar algún EDA antes de cualquier análisis sofisticado, para saber si los datos 
están suficientemente limpios, si hay valores perdidos o atípicos, y cómo están conformadas las distribuciones. Además, antes de realizar 
cualquier análisis multivariante o inferencial, necesitaremos conocer las frecuencias específicas de cada variable, sus medidas de 
tendencia central, su dispersión, etcétera. También es útil integrar las frecuencias de las distintas variables en una única tabla para 
tener una primera idea de sus interrelaciones.
...
una versión limpia de los datos de data[^1] almacenados en un archivo csv (utilizando la función read_csv de tidyverse en R y la 
...
::: {.callout-note icon=false collapse=true}
## pandas frente a solo 'numpy'/'scipy' 
...
de los valores de una lista, o la correlación entre dos listas. Qué utilizar depende de ti.
...
sustituyendo los valores que faltan por la media de la columna). Va más allá del alcance de este capítulo explicar todos los métodos de 
...
caja y cintas, y el trazado de datos geoespaciales. De hecho, la visualización de datos es un paso importante tanto en EDA como en el 
...
En el caso de los datos nominales, la forma más directa de visualizarlos es simplemente contar la frecuencia del valor y luego 
...
::: {#exm-bar}
Gráfico de barras del apoyo a los refugiados
...
la vista general (y la creación del gráfico podría consumir muchos recursos). En lugar de eso, agruparemos los datos en intervalos, 
como grupos de edad. De ahí que se utilicen histogramas para examinar la distribución de una variable continua 
...
discutir dos formas de hacerlo: trazando los datos a lo largo del tiempo y diagramas de dispersión para ilustrar la relación entre dos 
variables continuas.

El Eurobarómetro recoge datos durante 15 días (en el ejemplo, del 5 al 19 de noviembre de 2017) y puede ser interesante comprobar si 
el nivel de apoyo a los refugiados o incluso a los migrantes en general cambia con el tiempo. Por suerte, se trata de una serie 
temporal sencilla y puedes representarla con un gráfico de líneas. En primer lugar, debes utilizar una variable numérica para el 
nivel de apoyo ('support_refugees_n', que va de 1 a 4, siendo 4 el máximo apoyo) y agruparla por días para obtener la media de cada 
día. En el caso de R, puedes representar las dos series utilizando la función base 'plot', o puedes utilizar la función 'geom_line' de 
'ggplot2'. En el caso de Python, puedes utilizar la función 'plot' de 'matplotlib' o la función 'lineplot' de 'seaborn'. Empecemos por 
ver cómo el Ejemplo [-@exm-line] crea un gráfico sobre el apoyo medio a los refugiados por día.

::: {.callout-note appearance="simple" icon=false}

::: {#exm-line}
Gráfico de líneas sobre la ayuda media a los refugiados por día
...
y, en Python, utilizando la función 'scatterplot' de 'seaborn' (y añade 'lmplot' para incluir la línea de regresión como se muestra en 
...
las dos variables o, en otras palabras, cuanto mayor sea el apoyo medio a los refugiados, mayor será el apoyo medio a los inmigrantes 
...
(*X* e *Y*), produciendo un valor entre $-1$ y $+1$ donde 0 representa la ausencia de correlación y valores cercanos a 1, una 
...
de país-género. Se puede ver que el nivel de apoyo es similar en países como Eslovenia o España, y es diferente en la República Checa 
...
Como podrás ver, uno de los objetivos de EDA es explorar la varianza de nuestras variables, lo que incluye cierta incertidumbre 
...
entender y pueden tener un gran impacto en todo tipo de lectores, lo que los convierte en una representación muy útil para una amplia 
...
país) con sus coordenadas para disponer de un marco de datos preciso para trazar datos geoespaciales. Algunas bibliotecas en R y Python 
...
Hasta ahora, hemos revisado las técnicas estadísticas tradicionales de exploración y visualización que cualquier científico social 
...
en gran medida, una técnica inductiva y ascendente (véase el Capítulo [-@sec-chap-introduction] y
@Boumans2016).

En este capítulo, nos centraremos en UML como medio para encontrar grupos y dimensiones latentes en nuestros datos, lo que también 
...
En la minería de datos, utilizamos el análisis de grupos como técnica UML, de forma que su objetivo es encontrar la relación entre 
...
medio a los refugiados/migrantes, edad y nivel educativo. Podríamos crear algunos grupos a priori (como los países del sur frente a los 
...
también llamada medoide ($k$-medoides o partición en torno a medoides, PAM, por sus siglas en inglés) como forma de optimizar el 
método inicial.
...
número de grupos, $k$, de antemano. Esta es una cuestión delicada, y (al margen de decidir $k$ al azar), es necesario volver a estimar 
...
(es decir, de 20 a 50) para garantizar el máximo beneficio del método. La función base de R 'kmeans' y la función 'KMeans' de 
...
::: {#exm-kmeans}
Uso de Kmeans para agrupar países en función de la ayuda media a refugiados e inmigrantes, la edad y el nivel educativo
...
en un dendograma. Este algoritmo tiene dos versiones: un enfoque ascendente o aglomerativo (las observaciones comienzan en sus propios 
...
iterativamente la distancia entre cada nuevo grupo y los anteriores. Ésta es la regla interna del algoritmo y nosotros debemos elegir el 
...
necesitamos saber cuánta varianza (que es el cuadrado de la desviación típica) se explica con cada componente. Podemos obtener los 
...
y visualizar los grupos, y obtener una imagen muy similar a la obtenida en los ejemplos anteriores, con pequeñas diferencias como el 
