...
entradas; y 'Tensorflow' y 'keras' (ambos en Python y R) para construir y afinar una CNN.
...
una cantidad sin precedentes de contenidos multimedia que merecen ser tenidos en cuenta en muchos tipos de investigación.
...
embargo, también es cierto que las técnicas computacionales para analizar audio, imágenes o vídeo están aún poco desarrolladas en las 
...
ciertas aplicaciones, como la transformación de voz a texto, y realizando después análisis de texto (ya explicados en los capítulos 9, 10 
...
abordaremos cómo crear los modelos), nos sirve para mostrar cómo una aplicación específica, sencilla y potente del análisis automático 
...
femeninos del Congreso de Estados Unidos hablaban con mayor intensidad emocional cuando se referían a las mujeres.
...
(por ejemplo, Google, Amazon, Microsoft, etc.). Estos servicios comerciales ya han desarrollado sus propios modelos de aprendizaje 
...
adecuados como 'googleLanguageR' en R o 'google-cloud-language' en Python).
...
de Microsoft o Rekognition de Amazon. Por ejemplo, si te conectas a los servicios de Amazon Rekognition no solo podrás detectar y 
...
Tanto en este servicio como en el caso de Google Cloud, tendrás que obtener credenciales de venta comercial para poder conectarse a su API 
(aunque hay una "cuota" inicial gratuita de llamadas de acceso a la API antes de que se te exija pagar por su uso). Esta aproximación tiene 
...
almacenar y analizar imágenes a escala relativamente rápido utilizando servicios de computación en la nube.
...
sabiendo que la imagen trata sobre refugiados, algunas de estas etiquetas, asignadas por un ser humano para describir la imagen, resultarían 
bastante cínicas.
...
matriz de puntos que, a su vez, genera una 'bitmap image' (una imagen de mapa de *bits*) o un 'raster grafic' (gráfico rasterizado). La 
...
En el Ejemplo [-@exm-imagel], convertimos nuestra imagen JPG original a escala de grises y luego creamos un objeto con su representación 
...
Si no queremos utilizar píxeles, hay otras formas de almacenar imágenes digitales. Una de ellas es la de los gráficos vectoriales, con 
...
A veces, es necesario convertir la imagen a un tamaño específico. En el caso de la clasificación de imágenes, esto es especialmente importante, 
...
que vamos a necesitar para aplicar las funciones.
...
#| results: hide
#| cache: true
# Redimensiona y visualiza myimg1. Redúcela al 25%
...
## Código R
```{r resize2-r}
# Redimensiona y visualiza . Redimensiona a 224 x 224.
# ! se utiliza para especificar el ancho y alto exactos.
...
Habrás observado que la nueva imagen tiene ahora la anchura y la altura deseadas, pero está deformada. La razón es que la imagen 
...
Evidentemente, haciendo esto estás perdiendo parte de la información de la imagen, por lo que quizás quieras pensar en otras alternativas, 
...
::: {#exm-crop}
Función para recortar la imagen para crear un cuadrado y redimensionar la foto

::: {.panel-tabset}
## Código Python
```{python crop-python}
#| results: hide
# Corta y redimensiona a 224 x 224

# Adaptado desde Webb, Casas & Wilkerson (2020)
def crop(img):
    height = img.height
    width = img.width
...
duplicadas. Tomemos el caso de las imágenes RGB: ya en el Ejemplo [-@exm-imagergb] vimos cómo las podemos transformar en tres *arrays* 
...
# Crea 2 imágenes de 15 x 15 para compararlas
...
supervisada de textos explicada en el apartado [-@sec-supervised], pero usando imágenes en lugar de textos. Por ejemplo, si mostramos 
...
#| echo: false
# fetch_openml es demasiado lento y el caché no acaba de funcionar. Puedes intentar almacenar el caché de forma manual. 
import pickle
...
similar, $\sim$9--11\% cada una). Como hacemos en los ejemplos, se puede comprobar que las predicciones para las diez primeras imágenes 
...
otra hacia atrás) para obtener así un entrenamiento más eficiente.
...
Las bibliotecas de última generación para redes neuronales en general y para visión artificial en particular son 'TensorFlow'[^6] 
...
grises ($28\times 28$) de diez clases diferentes: “*ankle boots*” (botines), “*bags*” (bolsos), “*coats*” (abrigos), 
...
de entrada de 0--255 a 0--1 dividiéndolas por 255 para aplicar el descenso del gradiente. A continuación, obtenemos tres conjuntos con 
...
visión artificial. Además, para obtener mejores resultados, podríamos construir o explorar diferentes arquitecturas de CNN que puedan 
...
código abierto y propondremos un ejemplo de cómo ajustar una CNN existente para una investigación de ciencias sociales.
...
# Quita el canal extra para hacer la comparación
picture1 = picture1[,,-4] 
...
una nueva imagen enmarcada. A continuación, en el [-@exm-resnetpredictions2], volvemos a ejecutar la tarea de predicción 
...
[^12]: La descripción de los detalles técnicos de todas estas arquitecturas queda fuera del alcance de este libro, pero, además de poder recurrir a la literatura científica específica de cada arquitectura, algunos paquetes, como keras, incluyen documentación básica que puede serte de ayuda.︎
